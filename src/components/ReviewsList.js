...
const ReviewsList = ({ carinderiaId }) => {
    ...
    useEffect(() => {
        const fetchReviews = async () => {
            try {
                // Fetch reviews
                const reviewsRes = await fetch(`http://localhost:5000/api/feedback/${carinderiaId}`);
                const reviewsData = await reviewsRes.json();
                
                // Fetch average rating
                const ratingRes = await fetch(`http://localhost:5000/api/feedback/rating/${carinderiaId}`);
